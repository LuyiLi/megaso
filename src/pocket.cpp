...
void pocket::mainPocketRender()
{
	...
	for (int q = 0; q < 3; q++)
	{
		if (isOpened)
		{
			pocketUI_texture.render(170 + 50 * q, 220, MaterialPocketClip, 0, NULL, SDL_FLIP_NONE, 2);
			for (int i = 0; i < 6; i++)
			{
			...
			pocketUI_texture.render(20, 220, CraftTableClip, 0, NULL, SDL_FLIP_NONE, 2);
		}
	}
